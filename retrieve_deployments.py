...
dump_storage_template = "curl -s -XGET 'http://localhost:9200/cloudify_storage/node_instance,execution/_search?size=10000&q=deployment_id:{id}' > {tempfile}"
dump_events_template = "curl -s -XGET 'http://localhost:9200/cloudify_events/_search?size=10000&q=deployment_id:{id}' > {tempfile}"
...
with open(os.devnull, "w") as FNULL:
    for dep in deployments:
        dep_id = dep["id"]
        call(["cfy", "ssh", "-c", dump_storage_template.format(id=dep_id, tempfile=magic_path)], stdout=FNULL, stderr=FNULL)
        call(["cfy", "ssh", "-c", dump_events_template.format(id=dep_id, tempfile=magic_path2)], stdout=FNULL, stderr=FNULL)
