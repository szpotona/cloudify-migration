...
bulk_template = "curl -s XPOST 'http://localhost:9200/cloudify_events/_bulk' --data-binary @{file}"
bulk_entry_template = '{{ create: {{ "_id": "{id}", "_type": "{type}"  }} }}\n{source}\n'
    ...
    for dep in deployments:
        ...
        with open(os.devnull, "w") as FNULL:
            ...
            def remove_newlines(s):
                return s.replace('\n', ' ').replace('\r', '')
            bulk = "".join([bulk_entry_template.format(id=str(event["_id"]), type=str(event["_type"]), source=remove_newlines(json.dumps(event["_source"])) ) for event in events["hits"]["hits"]] )
            with open(host_magic_file, "w") as f:
                f.write(bulk)
            scp(host_magic_file, magic_file, True)
            call(["cfy", "ssh", "-c",  bulk_template.format(file=magic_file)], stdout=FNULL, stderr=FNULL)
